...
from .youtube import (
    extract_id_from_url,
    ensure_video_exists_on_youtube,
    download_metadata,
    fetch_transcript,
)
...
class GetVideoSponsorshipService:
    ...
    async def get_sponsorship_info(self, params: VideoSponsorshipRequest, session: AsyncSession):
        youtube_id = params.id or await extract_id_from_url(params.url)
        ...
        if video:
            ...
            youtube_id = await ensure_video_exists_on_youtube(youtube_id)
        ...
        if None in key_metadata.values():
            ...
            if not video_metadata:
                metadata_json = await download_metadata(youtube_id)
        ...
        if len(empty_subtitles_segments) > 0:
            ...
            if transcript is None:
                transcript, transcript_language = await fetch_transcript(youtube_id, video.language)
