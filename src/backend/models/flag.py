...
class Flag(Base):
    ...
    entity_flagged = Column(Enum(EntityType, inherit_schema=True), nullable=False, index=True)
    field_flagged = Column(String, nullable=False, index=True)
    value_flagged = Column(JSON)
    status = Column(Enum(FlagStatus, inherit_schema=True), default=FlagStatus.pending, index=True)
